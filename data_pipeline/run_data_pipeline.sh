...
# Archive old data_* folders (but not data_pipeline)
for old_data_dir in ../data_[0-9]*; do
...
log "Step 5/6: Cleaning and processing final dataset..."
...
log "Step 6/6: Preparing engineered features..."
FINAL_FEATURES_FILE="../$OUTPUT_DIR/final_features_$TIMESTAMP.csv"

$PYTHON_PATH feature_preparation.py \
    "$FINAL_METADATA_FILE" \
    "$FINAL_FEATURES_FILE"

if [ ! -f "$FINAL_FEATURES_FILE" ]; then
    error "Failed to create features file"
    exit 1
fi

FEATURES_COUNT=$(head -n 1 "$FINAL_FEATURES_FILE" | tr ',' '\n' | wc -l | tr -d ' ')
success "Feature engineering complete: $FEATURES_COUNT features"
