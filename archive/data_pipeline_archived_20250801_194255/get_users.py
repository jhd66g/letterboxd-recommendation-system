...
def get_popular_users(max_pages=128, time_period="this/week", max_users=None):
    """
    ...
    Args:
        ...
        max_users (int): Maximum number of users to scrape (optional)
    ...
    for page in pbar:
        ...
        try:
            ...
            table = soup.find("table", attrs={"class": "member-table"})
            
            if not table:
                print(f"Warning: No user table found on page {page}")
                continue
                
            rows = table.find_all("td", attrs={"class": "table-person"})
...
if __name__ == "__main__":
    ...
    parser.add_argument("-x", "--max-users", type=int, default=None,
                       help="Maximum number of users to scrape (overrides pages limit)")
    ...
    # Get users
    users = get_popular_users(max_pages=args.pages, time_period=args.time_period, max_users=args.max_users)
